var actorChars = { //STOPS IT FROM DRAWING
  '@' : Player,
  'o' : Coin,
  'Q' : Coin2
};
function Level(plan) {
  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      var Actor = actorChars[ch]; //CHECK VARIABLE CAPITALIZATION
      if (Actor)
...
Level.prototype.actorAt = function(actor) { //IF I HAVE BUGS CHECK THIS
...
Player.prototype.moveY = function(step, level, keys) {
  ...
  this.speed.y += step * gravity; //THIS MOVES DOWN
  var motion = new Vector(0, this.speed.y * step);
  var newPos = this.pos.plus(motion);
  var obstacle = level.obstacleAt(newPos, this.size);
  //The floor is also an obstacle
  //jump f ther are touching an object
  if (obstacle) {
